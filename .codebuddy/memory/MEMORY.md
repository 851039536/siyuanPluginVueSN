...
- ⚠️ **共享弹层不用 Teleport**（就地 `position: fixed`，遮罩 `inset: 0`）⇒ **任何带 transform / filter / contain 的祖先都会成为它的包含块，把弹层关进容器并裁剪**。实测：toolCollection 面板曾用 `left: 50% + translateX(-50%)` 居中 + `contain: layout paint` ⇒ 工具内 `Dialog` / `ConfirmDialog` 错位且被裁。修法：容器居中改 `left/right: 0` + `margin-inline: auto`，动画 transform 只留在过渡类内，并移除 `contain`（工具栏面板 = `.tool-collection-panel`，2026-09-14 CR-009 已修，勿回退）
- ⚠️ **工具合集全局键盘导航**（`useToolNavigation.handleKeydown`）：已排除输入类元素（INPUT / TEXTAREA / SELECT / contentEditable），但**弹层**要另判 —— 现加 `document.querySelector('[aria-modal="true"]')` 命中即 return（Esc / 方向键归弹层所有）。判定可靠：共享 `Dialog` 仅在 `modal` 为真时置 `aria-modal="true"`（非模态为 `"false"`），`ConfirmDialog` 恒为 true
...
- ⚠️ **共享 `Select` / `Input` 是纯受控组件**（内部只 `emit("update:modelValue")`，显示完全取自 `props.modelValue`）：写 `:model-value` + `@update:model-value="handler"` 时**必须在 handler 里自己回写**受控值（或用 `v-model` / 父级 v-model 回写），否则选中后触发器文案与下拉勾选态永远停在旧值 —— shortcut「分组」下拉曾因此看起来"选什么都没生效"（底层数据其实已更新，只有显示没变）
- ⚠️ **用 `v-if` 在两种控件间切换时必须手动移交焦点**：新控件不会自动获焦，用户直接敲键盘会**静默落空**（输入框实际是空的）；若此时叠加「留空即回落默认值」的静默兜底，就会表现为「新建 XX 没效果」—— shortcut 的「新建分组」曾如此。修法是 `await nextTick()` 后调子组件暴露的 `focus()`（`Input` / `Button` 均有 `defineExpose({ focus })`），并给「用户的显式选择」补就地校验错误而非静默回落
...
## 9. 功能模块速查
- **shortcut**（2026-09-14 完全重构 + **CR-009 迁入工具合集**：现路径 `src/features/toolCollection/tools/shortcut/`，规格仍在 `docs/shortcut-refactor-spec.md`；注册于 `tools/registry.ts` 第 10 个工具，独立 Dock 与 `enableShortcuts` 设置项已摘除）：**卡片网格（列数随容器自适应 `auto-fill, minmax(220px, 1fr)`：窄容器 2 列 / 底部面板约 4 列 / 独立窗口 5~6 列）**（**内容行**：主内容自适应〔`resolveShortcutDisplay` 判定 `keys` → 按键徽章组 / `code` → 等宽代码芯片〕+ 弱化次显快捷键徽章 + 悬停浮出的操作按钮〔`visibility` 恒定预留空间 ⇒ 零抖动〕；**名称行**：名称/平台标签/描述〔`flex-wrap` + 描述 `flex-basis: 120px` ⇒ 名称长时描述自动折到第二行〕；分组吸顶 + 会话级折叠）+ **单键持久化**（`plugin-shortcuts-custom` 唯一可写；旧键 `plugin-shortcuts-all` 仅迁移源，**守卫用新键 `exists()` 含空数组**、写成功才删旧键）+ 导入导出/重置 + 分组下拉（`utils.listGroups` 派生现有分组 + 「新建分组…」哨兵项切输入框）。**已移除**：收藏（CR-003）、4 类预设（CR-004，仅留 NPM/NVM/Visual Studio 共 42 条；分类标识与 i18n 文案保留 ⇒ 补回 `data/<分类>.ts` 即可恢复）、最近使用与冲突检测（CR-006 ⇒ 筛选只剩「搜索 + 分类」，`ShortcutQuery` 仅 `keyword`/`category`；i18n 键 `favorite`/`unFavorite`/`filterFavorite`/`filterRecent`/`scFilterConflict` 因扁平命名空间的跨模块读取风险一律保留未删）。分层：`utils.ts`/`dataTransfer.ts` 纯函数 → `manager.ts` 双段（预置只读 + 自定义可写，`isPreset` 双层设防）→ `composables/useShortcutData|useShortcutFilter` → `index.vue` 编排。**CR-007 要点**：显示模型收敛为纯函数 `isKeyCombo`/`resolveShortcutDisplay`（`content = copyContent || keys`；`hotkey` 仅当「有独立 copyContent + 与 keys 不同 + keys 是按键组合」；拆序列只在 `kind === "keys"` 时做，含逗号命令不会被拆散），**复制与显示同源**；表单单字段「按键组合」→「内容（必填 → copyContent）+ 快捷键（可选 → keys，留空写内容副本）」；`ShortcutInfo` 结构不变 ⇒ **零迁移**（`sanitizeShortcutArray` 加「keys 缺失回退 copyContent」防御）；搜索覆盖 copyContent。**模块样式审计结论：废代码 0**（未使用类 0 / 未引用局部变量 0 / 已删功能残留 0 / Token 违规 0），实际动作是等价改写（芯片族骨架与省略号组合并、`$motion-fast` 常量、两处依赖共享组件类名的锚点注释）。**CR-008**：`ShortcutInfo.keys` 放开为**可选**（命令行类条目无原生快捷键）⇒ `data/npm.ts` / `data/nvm.ts` 删除全部编造的 `Ctrl+Alt+X` 伪键位（各 10/9 条，仅 `visualStudio.ts` 的 23 条真键位保留）；配套兜底两处：`searchShortcuts` 的 `s.keys` 可选链、`ShortcutDialog` 回填 `keys ?? ""`。**CR-009 要点**：23 文件 `git mv` 迁入 `tools/shortcut/`（分层不变）+ 新增 `bootstrap.ts`（`ensureShortcutData(plugin)`，模块级 Promise 缓存保证切工具重建组件时只初始化一次、失败不缓存失败态）；工具契约 `{ plugin, i18n }`，此处 `plugin` **必填**；存储键改名 `plugin-toolCollection-shortcut-custom` 并做**两代迁移**（`plugin-shortcuts-custom` 优先 → `plugin-shortcuts-all` 兜底，逐代命中即停、旧键为空数组即视为已清空、写新键成功才删旧键）；注册链摘除按 unitConverter/wordQuery 先例留注释占位，并移除 `vite.config.ts` / `tsconfig.json` 的 `@shortcut` 别名与 `AGENTS_API.md` 别名清单条目。
  ⚠️ 迁移类改动的完整清单（照 unitConverter/wordQuery 先例）：① `git mv` 整目录 ② `tools/registry.ts` 加 `TOOL_REGISTRY` 条目 + `TOOL_LABEL_KEYS` ③ 删 `src/index.ts` 调用与 import、`features/index.ts` 导出、`settings.ts` 开关、i18n `enableXxx(Desc)` 两键（留「已迁移至 …」注释，再跑 `pnpm i18n:merge`）④ 删 `@<feature>` 别名（`vite.config.ts` + `tsconfig.json` + `AGENTS_API.md` 别名清单）⑤ i18n 分片**保留不合并**（工具靠全局扁平键读取）。新增工具无需改容器 `index.vue`（registry 驱动），`useToolReorder` 会把不在持久化顺序里的新工具追补到末尾；`Ctrl+1~9` 只覆盖前 9 项。
