...
- **共享组件库使用规则**（2026-09-10，已写入 `AGENTS.md § 共享组件库使用规则（强制）`）：①**先查用法禁止猜 props** —— 查组件预览面板（推荐）/ `previewData/*.ts` 清单 / 组件源码 `interface Props`，图标只能传已注册 `IconKey`，尺寸档位统一 `xsmall/small/medium/large`（默认 small）；②**优先复用禁止 feature 自建同类控件**（按钮/输入框/下拉/开关/滑块/标签/徽标/头像/卡片/图表/图标/加载态 12 类），缺能力时**先扩展共享组件**（加可选 props 保持向后兼容）；例外仅纯展示布局容器与无档位 26×26 `.icon-btn`；③**改组件 API 必须同步 `previewData/*.ts` 示例（props 与 code 一致）+ `componentPreview/README.md`**。`src/components/` 共 14 个组件，清单表见 AGENTS.md
