...
- gitPush 提交规则检查共 11 条规则（2026-09-07 扩展）：原 7 条 + 新 4 条（invalidScopeFormat scope 格式非法 / subjectEndsWithPeriod 句号结尾 / subjectTooShort 描述过短 / missingBlankLine 正文未空行）。架构要点：checkCommitRule 等纯函数用可选参数 `config: CommitRuleConfig = DEFAULT_COMMIT_RULE_CONFIG` 向后兼容 6 处调用点；阈值持久化在 ruleCheckPrefs 槽位 `minSubjectLength?` 可选字段（默认 10，clampMinSubjectLength 钳位 1~100）；setRuleCheckProject/setMinSubjectLength 均 load-merge-save 防覆盖；subject 按首个换行拆分，长度/句号/中文判定只作用于首行；启发式修正结果经 checkCommitRule 终验，仅返回完全合规结果（过短/未空行走 AI）；设置入口 SettingsDialog 常规分区「描述最短字数」，index.vue init 中 `void loadRuleCheckPrefs()` 预载。AI prompt 同步（同日）：commitRuleChecker 导出 buildCommitRulePrompt(minSubjectLength) 作为规则约束 prompt 单一事实源，CommitMsgGenerator.getRuleConfig() 读 ruleCheckPrefs 阈值，3 处 AI prompt（generateCommitMessage/generateCommitFix/deepAnalyzeCommitFix）注入约束 + 生成后 checkCommitRule(trimmed, config) 同口径校验 + heuristic 同 config；generateCommitMessage 后处理去尾句号、maxTokens 60→100；示例文本已改为符合新规则（原"修复订单列表空指针异常"12 字过短、feat(gitPush) scope 含大写均会被新规则拦截）；CommitFixDialog/BatchFixDialog 实时校验同口径（onMounted 读阈值）；WorkingTreePanel/ProjectCard 无 manager 保持默认 15（提交框提示性校验宽松方向，AI 生成更严格不会冲突）
