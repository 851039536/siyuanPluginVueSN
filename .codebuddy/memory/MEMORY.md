...
- **gitPush**：行数口径 = 工作区存量（`git ls-files` + 逐文件 readFileSync）；多本地路径 `resolveValidPath`；提交规则经 `readCommitRuleConfig(prefs)` 单一入口；⚠️ `styles/` 目录树计数与 `common/` 清单属历史欠账，改该文件时计数一律实测。差异弹窗（2026-09-14）：`getFileDiff` 对未跟踪文件用 `git ls-tree HEAD` 判据 + `git diff --no-index -- /dev/null` 兜底（退出码 1 需 `execGit` 的 `allowExitCodes`）；`FileChange.unstaged` 标记两份差异；`utils.diffCacheKey` 统一缓存键；`utils.ts` 已拆为 `utils/`（12 文件 + index.ts 汇聚，最大 `diffText.ts` 206 行）、`WorktreeOps.ts` 的 git 输出解析抽到 `utils/gitOutput.ts`（485 行）；`FILE_STATUS_META` 只存 `titleKey`（文案经 `fileStatusText/fileStatusTitle(file, i18n)`）；审查修复：`discardFile` staged 分支按 `isFileInHead` 分流（新增类 reset 后 `clean -f`，原 checkout 对不在 HEAD 的文件必报 pathspec 错）、diff 兜底判据按基线区分（staged=false 判 `isFileInIndex`、staged=true 判 `isFileInHead`）
...
- **共享控件迁移（2026-09-14，gitPush/ListView 全量完成）**：① `Button` 新增 **`dense`** 修饰（`si-button--dense`，**仅与 `size="xsmall"` 协同**：去 min-height、padding 2px 5px、gap 3px、圆角 4px、纯图标 20px；独立使用无效果）→ 紧凑按钮统一写 `<Button variant="…" size="xsmall" dense>`，`ToggleButton` 已透传 `dense`；② 覆写共享组件的 feature 规则**统一抬 (0,3,0)～(0,4,0)**（`.git-push-panel`/直接容器类作前缀）并删 `!important`，涉及 padding/圆角的覆写必须 (0,4,0)（与 `--dense.si-button--xsmall[data-v]` 同为 (0,3,0) 会打平）；③ `Dialog` 用 **header 插槽**时必须把作用域 `headerId` 打到自己的标题元素上，否则 `aria-labelledby` 悬空；Esc 交 Dialog 内建，参数化键（←/→）保留捕获阶段 `stopImmediatePropagation`；④ `Tabs` 面板走**具名插槽**（`<slot :name="tab.id" />` 在 `TabPanel` 内）可获得真实 tabpanel 语义且 `lazy` 仍生效；⑤ **删 `import { Icon }` 前必须 grep 模板 `<Icon`**——残留时 `read_lints` 与 `pnpm typecheck` 都不报（组件名落进全局 shim）。报告与例外登记：`docs/gitPush-listview-controls-review.md`；`Buttons.scss` 保留（gitPush 其余 27 文件仍用 `vp-btn`，其 0.15s 过渡属范围外残留）
