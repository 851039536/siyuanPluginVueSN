...
> 最后整理：2026-09-10（合并重复条目、压缩模块状态，新增共享组件库第 15 个组件 Checkbox）
...
## 共享组件库（`src/components/`，15 个）
规则见 `AGENTS.md § 共享组件库使用规则（强制）`：**先查用法（组件预览面板 / `previewData/*.ts` / 源码 `interface Props`）→ 优先复用（12 类控件禁止 feature 自建）→ 改 API 必同步预览清单**。
- 清单：Button / Input / Select / FormField / Label / Switch / **Checkbox** / Slider / Tag / Badge / Avatar / Card / Chart / IconWrapper / Loader
...
- 具名/作用域插槽**无法**在预览快照渲染，必须在 `componentPreview/README.md` 登记
...
- 禁止在 feature 内自建共享组件已覆盖的 12 类控件
