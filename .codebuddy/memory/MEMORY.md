...
- **面板第二轮：舞台分档 + 懒挂载（2026-09-12）**：`STAGE_CLASS_BY_GROUP` 已改为 **`Record<string, string[]>`（可叠加）**，分档依据是**组件根元素的宽度/高度特性**——`--compact`(64px，原子控件 20~44px 高)、`--fill`(Card/Panel/Message/Paginator 根**无 width**，会被舞台行 flex 收缩成内容宽 ⇒ 满宽+顶对齐，写法 `> * { width:100% }` 只作用于组件根)、`--top`(Timeline/Tabs/FileUpload/Listbox/Splitter 高度多变)、`--chart`(≥190px)；列宽下限 230→**260px**；跨列名单追加 **toolbar / paginator**。**懒挂载 + 远区卸载**：`index.vue` 单 IO（`root=contentRef`、`rootMargin: 800px`）进区挂载 / 出区 400ms 后卸载，分区外壳常驻、卸载用**网格实测高度**撑 `.cp-section__placeholder`，首帧由 `mountSectionsInViewport()` 同步挂载，锚点跳转与搜索命中先 `forceMount`。卡片本体拆到 `PreviewCard.vue`（`resolvedProps` computed 只解析一次，本体与复合示例共用同一对象），受控值由 `PreviewStage.ts` 持有并带**同值写回守卫**（`Object.is` + 数组内容浅比较）—— 这是截断 `emit→写回→重渲染→emit` 自激循环的关键。内存排查手册：`docs/component-preview-memory-diagnosis.md`（三脚本 + 判定树 + 二分法）。
- **面板样式归属（第三轮去重后，2026-09-12）**：`styles/index.scss`（面板外壳/头部/档位切换器/内容区）**只由根 `index.vue` 引入一次**，子组件**不要再引它**（否则同一批选择器在产物重复数份）；`PreviewSection.scss` = 分区级（外壳/import 行/网格/跨列/卸载占位，8 类）；`PreviewCard.scss` = 卡片级（卡片 + 舞台与分档 + 弹层沙箱覆盖 + 全部 `.cp-*-demo*` 演示宿主类，42 类）；`NavSidebar.scss` / `CodeBlock.scss` 各归其组件。另：滚动高亮已 rAF 合并且只在 `activeId` 变化时写入；`isSameValue` 已覆盖 `Date`（比时间戳）与**仅普通对象**的逐键浅比较（`File`/`Blob`/`Map` 保持引用比较）
