...
- **字号层级规范**：所有 feature 的 UI 一律采用两级字号制——标题与正文内容 `$font-size-xs`(12px)，辅助文字（标签、提示、描述、状态、命令输出、元信息）`$font-size-2xs`(10px)。`$font-size-sm`(14px) 及以上仅限阅读区正文（如 Markdown 预览）与数据突出展示（如统计数值），且必须加注释说明用途。所有 Dock 面板根容器与弹窗遮罩层（mask/overlay）必须显式设置基准 `font-size: $font-size-xs`，防止内部未声明字号的文字继承思源全局默认字号（14px+）导致偏大。参考实现：`src/features/gitPush/styles/`
