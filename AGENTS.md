...
`src/components/` 是**全项目唯一的 UI 控件来源**（46 个组件）。三条强制要求：**先查用法 → 优先复用 → 改 API 必同步**。
...
| 查询方式 | 位置 | 说明 |
|---------|------|------|
| **组件预览面板（推荐）** | 命令面板搜「组件预览」/ 状态栏功能列表 | 46 个组件的**真实渲染**快照 + 可复制代码；改完组件样式可直接目视回归 |
...
├── components/             # 共享组件库 46 个（ 等）— 使用规则见「共享组件库使用规则」
├── features/
│   ├── statusBar/
│   │   └── composables/
...
| [src/features/componentPreview/README.md](./src/features/componentPreview/README.md) | 共享组件预览面板机制（46 个组件的用法快照、受控示例可交互、组件尺寸档位、`sizeable`/`resolveProps`、复合示例 `render`、具名/作用域插槽 `slots`、弹层类沙箱覆盖、清单扩展指南） | 使用共享组件前查用法、或改共享组件 API 后同步预览清单时 |
