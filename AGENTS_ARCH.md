...
单元测试文件与**被测源文件同目录**，命名 `<源文件名>.spec.ts`（如 `src/utils/format.spec.ts` 对应 `src/utils/format.ts`）。
禁止建 `test/` / `tests/` / `__tests__/` 集中目录 —— 集中放置会让「改实现要找测试」产生跨目录跳跃，而同目录紧邻便于对照维护。

| 项 | 约定 |
|----|------|
| 命名 | `<源文件名>.spec.ts`，前缀与被测文件完全同名 |
| 位置 | 与被测文件同目录 |
| 内容 | 仅纯函数测试（纳测标准与「明确不做」清单见 [AGENTS_BUILD.md § 单元测试](./AGENTS_BUILD.md#单元测试)） |
| 文件头注释 | 同「强制规则：文件头注释」（`// 文件功能说明`） |
| 构建影响 | 无 —— `*.spec.ts` 从构建入口 `src/index.ts` 不可达，不会被打包 |
| 行数上限 | 不适用「单文件行数上限」—— 测试用例数量由被测函数的边界数决定，不因超 500 行而拆 |

**纳入范围（强制）**：改动已被 spec 覆盖的纯函数时，必须同批更新对应 spec。
