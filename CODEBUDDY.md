...
- **请求加载反馈**：发起请求（AI 调用、网络请求、耗时操作）时，触发按钮图标切换为环形加载图标 `mdi:loading` 并旋转（`animation: spin 1s linear infinite`），请求期间 `disabled` 防重复提交，结束（成功/失败/超时）后还原图标与禁用态。旋转动画放 SCSS 文件，禁止内联 style 硬编码；图标需在 `icons.ts` 已注册。详见 [AGENTS_RULES.md § 强制规则：请求中的加载反馈](./AGENTS_RULES.md#强制规则请求中的加载反馈)
