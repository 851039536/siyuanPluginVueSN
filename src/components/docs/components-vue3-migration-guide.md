...
> 这套 UI 组件库（按钮、输入框、下拉、日历、图表等 **36 个组件**）原本跑在思源笔记里。
> 本文档告诉你怎么把它搬到一个**普通 Vue 3 项目**里用。
>
> **基线**：2026-09-11，`src/components/` 共 109 个文件（36 个公开组件 + 各组件私有子目录 + `styles/` + `kit/` 支撑目录与本说明文档）。
...
- **36 个组件**：Avatar / Badge / Button / Card / Chart / Checkbox / ColorField / ConfirmDialog /
  DatePicker / Divider / FormField / IconWrapper / Input / InputGroup / InputGroupAddon / Label / Listbox /
  Loader / Paginator / Panel / RadioButton / Select / Slider / SpeedDial / Splitter / SplitterPanel / Switch / Tab / TabList / TabPanel / TabPanels / Tabs / Tag / Textarea / Timeline / ToggleButton
...
| `Failed to resolve import "./datePicker/PickerPanel.vue"` | 私有子目录（`datePicker/` `select/` `speedDial/` `paginator/` `textarea/` `timeline/` `splitter/` `tabs/`）没带全 | 同上 |
...
- 36 个公开组件内部只用相对路径互相引用，并各带一行 `import "./kit/theme"`
