...
在思源内以独立窗口/页签形态查看共享 Codex UI 组件库（`src/components/`）全部 25 个组件的真实渲染用法快照，附可复制的示例代码，便于组件开发者查看效果、改动后快速回归验证。
...
- **全组件覆盖**：Avatar / Badge / Button / Card / Chart / Checkbox / ColorField / ConfirmDialog / DatePicker / FormField / IconWrapper / Input / InputGroup / Label / Listbox / Loader / RadioButton / Select / Slider / SpeedDial / Switch / Tag / Textarea / ToggleButton 各一个分组分区（`InputGroup` 与 `InputGroupAddon` 为配套组件，共用「InputGroup」一个分区），分组内为典型 props 组合快照卡片。
