...
export function isNetworkSlow(): boolean {
  ...
  if (typeof navigator !== 'undefined' && (navigator as any).connection) {
    ...
    cachedNetworkSlow = ['slow-2g', '2g', '3g'].includes(connection.effectiveType)
  } else {
...
export function isCacheValid<T>(cacheData: CacheData<T> | null | undefined, cacheExpiryTime: number): cacheData is CacheData<T> {
...
export async function copyToClipboard(text: string, i18n: DiskBrowserI18n): Promise<boolean> {
  try {
