<template>
  ...
  >
    ...
    <div class="doc-badges">
      <span class="badge time-badge" v-if="doc.updated" :class="timeClass">
        <Icon icon="mdi:clock-outline" class="badge-icon" />
        {{ formatTime(doc.updated) }}
      </span>
      <span class="badge depth-badge" v-if="doc.depth !== undefined && doc.depth >= 3">
        <Icon icon="mdi:sitemap-outline" class="badge-icon" />
        {{ doc.depth }}层
      </span>
      <span class="badge ref-badge" v-if="doc.refCount && doc.refCount > 0">
        <Icon icon="mdi:link-variant" class="badge-icon" />
        {{ doc.refCount }}引用
      </span>
      <span class="badge img-badge" v-if="doc.imageCount && doc.imageCount > 0">
        <Icon icon="mdi:image-outline" class="badge-icon" />
        {{ doc.imageCount }}
      </span>
    </div>
    <div class="doc-size">
...
function parseSiyuanTime(ts: string): Date | null {
  ...
  const hour = ts.length >= 10 ? parseInt(ts.substring(8, 10)) : 0;
  const min = ts.length >= 12 ? parseInt(ts.substring(10, 12)) : 0;
  const sec = ts.length >= 14 ? parseInt(ts.substring(12, 14)) : 0;
