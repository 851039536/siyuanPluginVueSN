...
- **发布状态徽章**：导航栏面包屑旁以文字展示当前文档发布状态（已发布绿、未发布灰）；悬停或点击文字展开浮层显示所有发布平台；判定标准与 docAnalysis 一致——存在任一 `custom-<平台>-yaml` 属性（值非空）即视为已发布（`getBlockAttrs` 读取属性，`getPublishedPlatformNames` 按 `PLATFORM_MATCHERS` 识别平台，随 `DocMeta` 一并缓存）；由 `DocNavSettings.enablePublishStatus` 开关控制（默认开启，可在导航栏关键词编辑面板（铅笔图标）中切换）
