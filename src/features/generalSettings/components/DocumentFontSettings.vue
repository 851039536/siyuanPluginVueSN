...
async function loadSettings() {
  ...
  try {
    ...
    if (data) {
      ...
      // 同步下拉选中：当前字体命中预设列表时显示对应项，保证内置字体提示与实际一致
      const matched = PRESET_FONTS.find((opt) => opt.value === settings.value.fontFamily)
      presetFont.value = matched ? String(matched.value) : ""
    }
  } catch (error) {
