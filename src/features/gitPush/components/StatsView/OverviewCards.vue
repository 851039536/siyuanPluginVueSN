...
<template>
  <div class="gps-section">
    ...
    <!-- 推送状态概要：待推送/待拉取/已同步/无远程（与上方「待推送」卡片同源，此处作四态分布展示） -->
    <StatusChipBar :chips="statusChips" />
  </div>
</template>
...
// 推送状态 chip 配置：待推送/待拉取/已同步/无远程（labelKey 取现有 i18n 键作 hover 提示）
const STATUS_CHIPS = [
...
/** chip 视图：从推送状态统计取数，hover 文案在此按 i18n 预渲染 */
const statusChips = computed(() =>
  STATUS_CHIPS.map((c) => ({
    ...
    value: props.stats.pushStatusStats[c.key],
  })),
)
</script>
