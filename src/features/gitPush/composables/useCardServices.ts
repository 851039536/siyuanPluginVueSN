...
export function useCardServices(project: () => GitProject): {
  ...
  const stashLoading = computed(() => byId(records.stashLoading.value) ?? false)
  ...
  const gitOpLoading = computed(() => byId(records.gitOpLoading.value) ?? false)
  ...
  const fetching = computed(() => byId(records.fetching.value) ?? false)
  const remoteStatusLoading = computed(() => byId(records.remoteStatusLoading.value) ?? false)
  const refreshingWorkingTree = computed(() => byId(records.refreshingWorkingTree.value) ?? false)
