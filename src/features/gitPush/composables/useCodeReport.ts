...
export function useCodeReport(manager: GitPushManager, projects: Ref<GitProject[]>, i18n: Record<string, any>) {
  ...
  async function runReport() {
    ...
    try {
      ...
      reportData.value = gitFailed
        ? buildEmptyReport(rangeLabel)
        : buildReportData(project, commits, rangeLabel)
  ...
  async function fetchFilePatch(path: string): Promise<string> {
    ...
    const cwd = resolveValidPath(project)
    try {
      return await manager.getFileHistoryPatch(cwd, path, sinceForRange(range.value) || undefined)
    } catch {
      return ""
    }
  }
