...
在右侧边栏以 **紧凑列表** 展示与管理快捷键：内置思源笔记、插件自身、OpenSpec、NPM / NVM / CMD / VS Code / Visual Studio 等 89 条预置快捷键，并支持用户自定义增删改、收藏、最近使用、冲突检测与数据导入导出。
...
- **操作按钮悬停浮出**：收藏 / 复制（自定义项另有编辑 / 删除）默认隐藏但**恒定预留空间**（`visibility: hidden` 不参与布局）⇒ 悬停与键盘聚焦只做淡入，卡片不会抖动；收藏态下常显。
- **状态提示**：冲突卡警告色淡底 + 左侧警告色竖条（气泡列出冲突条目）；收藏卡极浅主色底 + 主色描边；最近使用项名称前有主色圆点。
...
三键分离：**预置不落盘**（代码即唯一真源，插件升级后新增预置立即可见），仅用户数据写盘。

| 存储键 | 内容 | 说明 |
|--------|------|------|
| `plugin-shortcuts-custom` | `ShortcutInfo[]` | 唯一可写的自定义快捷键段 |
| `plugin-shortcuts-favorites` | `string[]` | 收藏 id |
...
│   └── storage.ts               # 三键存储 + 幂等迁移 + 剪枝 + 重置
├── composables/
│   ├── useShortcutData.ts       # 响应式镜像、增删改、收藏/最近、导入导出、重置
│   └── useShortcutFilter.ts     # 搜索 / 分类 / 收藏 / 最近 / 冲突筛选管道
