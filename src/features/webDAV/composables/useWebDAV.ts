...
export function useWebDAV(i18nRef: { value: Record<string, any> }) {
  ...
  const isConfigured = computed(() => {
    return state.fileList.length > 0 || (state.currentPath && state.connectionStatus === 'connected')
  })
  ...
  const fetchFileList = async (config: WebDAVConfig, path?: string) => {
    ...
    try {
      ...
      addLog('info', i18n?.fileListLoaded || `已加载文件列表，共 ${files.length} 个项目`)
    } catch (error) {
      state.currentPath = path || config.basePath || '/'
      const errorMessage = error instanceof Error ? error.message : String(error)
      addLog('error', i18n?.loadFileListError || `加载文件列表错误: ${errorMessage}`)
    } finally {
