 ...
 * amz 时间戳生成、ListObjects/错误响应 XML 解析（含 delimiter 目录式解析）、
 * 请求体分块写入与 NodeResponse 兼容接口定义。
 * 从 s3Backup/modules/s3Protocol.ts 提升，供多个功能模块共用。
 */
...
/** 数字补零（如 pad2(3) → "03"），协议层私有实现避免反向依赖功能模块 */
...
/** ListObjects + delimiter 的目录式单页解析结果 */
export interface ListDirPage extends ListObjectsPage {
  /** 子目录前缀（<CommonPrefixes><Prefix>，以 / 结尾） */
  folders: string[]
}

/**
 * 解析开启 delimiter=/ 的 ListObjects XML 响应
 * 在 parseListObjectsXml 基础上额外提取 CommonPrefixes 作为子目录列表；
 * 开启 delimiter 后分页 NextMarker 可能指向 CommonPrefix，翻页逻辑由调用方处理
 */
export function parseListDirXml(xml: string): ListDirPage {
  const base = parseListObjectsXml(xml)
  const folders: string[] = []
  const re = /<CommonPrefixes>[\s\S]*?<Prefix>([\s\S]*?)<\/Prefix>[\s\S]*?<\/CommonPrefixes>/g
  let m: RegExpExecArray | null
  while ((m = re.exec(xml)) !== null) {
    folders.push(unescapeXml(m[1]))
  }
  // 开启 delimiter 时若缺 NextMarker，取最后一个 key 或最后一个 CommonPrefix 中较大者作为续传点
  let nextMarker = base.nextMarker
  if (base.isTruncated && folders.length > 0) {
    const lastFolder = folders[folders.length - 1]
    if (!nextMarker || lastFolder > nextMarker) {
      nextMarker = lastFolder
    }
  }
  return { ...base, folders, nextMarker }
}
